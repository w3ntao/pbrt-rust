...
const SWATCH_REFLECTRANCES: [ConstPieceWiseLinearSpectrum<38>; NUM_SWATCH_REFLECTANCES] = [
    ConstPieceWiseLinearSpectrum::from_interleaved_missing_short_and_long_wavelengths(
        [
...
impl PixelSensor {
    pub fn create(
        ...
        let iso = parameters.get_one_float("iso", Some(100.0));
        let sensor_name = parameters.get_string("senseo", Some("cie1931".to_string()));
        ...
        let white_balance_val = {
            let wb = parameters.get_one_float("whitebalance", Some(0.0));
            if sensor_name != "cie1931" && wb == 0.0 {
                6500.0
            } else {
                wb
            }
        ...
        let d_illum = DenselySampledSpectrum::cie_d(if white_balance_val == 0.0 {
            6500.0
        } else {
            white_balance_val
        });

        let sensor_illum: Option<Arc<dyn Spectrum>> = if white_balance_val != 0.0 {
    ...
    pub fn to_sensor_rgb(&self, l: &SampledSpectrum, lambda: &SampledWavelengths) -> RGB {
        let spectrum = l.safe_div(&lambda.pdf_as_sampled_spectrum());

        return self.imaging_ratio
            * RGB {
                r: (self.r_bar.sample(lambda) * spectrum).average(),
                g: (self.g_bar.sample(lambda) * spectrum).average(),
                b: (self.b_bar.sample(lambda) * spectrum).average(),
            };
    }
}
